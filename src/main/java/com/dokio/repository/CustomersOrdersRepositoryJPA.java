...
//        } else return 0;
    ...
    @SuppressWarnings("Duplicates")
    public List<CustomersOrdersProductForm> getCustomersOrdersProductTable(Long docId) {
        if(securityRepositoryJPA.userHasPermissions_OR(23L, "287,288,289,290"))//(см. файл Permissions Id)
        {
            String stringQuery;
            boolean needToSetParameter_MyDepthsIds = false;
            ...
            List<Object[]> queryList = query.getResultList();
            List<CustomersOrdersProductForm> returnList = new ArrayList<>();
            for(Object[] obj:queryList){
                CustomersOrdersProductForm doc=new CustomersOrdersProductForm();
                ...
                doc.setReserved((Boolean)                                obj[13]);
                returnList.add(doc);
            }
            return returnList;
        } else return null;
...
//*****************************************************************************************************************************************************
    ...
    public Long insertCustomersOrders(CustomersOrdersForm request) {
        if(securityRepositoryJPA.userHasPermissions_OR(23L,"280,281,282"))//  "Создание"
        {
            CustomersOrders newDocument = new CustomersOrders();
            ...
            {   // проверка на то, что предприятие, для которого содается документ, наодится под главным аккаунтом
                ...
                // ФИО заказчика
                newDocument.setFio(request.getFio());
                // email
                newDocument.setEmail(request.getEmail());
                // контактный телефон
                newDocument.setTelephone(request.getTelephone());
                // почтовый индекс
                newDocument.setZip_code(request.getZip_code());
                // Страна
                if (request.getCountry_id() != null) {
                    newDocument.setCountry(emgr.find(Countries.class, request.getCountry_id()));
                }
                // Область
                if (request.getRegion_id() != null) {
                    newDocument.setRegion(emgr.find(Regions.class, request.getRegion_id()));
                }
                // Населенный пункт
                if (request.getCity_id() != null) {
                    newDocument.setCity(emgr.find(Cities.class, request.getCity_id()));
                }
                // Уточнение к адресу (как вариант, можно весь адрес писать здесь)
                newDocument.setAdditional_address(request.getAdditional_address());
                // трек-номер
                newDocument.setTrack_number(request.getTrack_number());
                // статус документа
                newDocument.setStatus(emgr.find(SpravStatusDocks.class, request.getStatus_id()));
    ...
    @Transactional
    public  Boolean updateCustomersOrders(CustomersOrdersForm request) {
        ...
        {
            if(updateCustomersOrdersWithoutTable(request)){                                      //метод 1
                try {//сохранение таблицы
                    String productIds = "";
                    if (request.getCustomersOrdersProductTable().size() > 0) {
                        for (CustomersOrdersProductForm row : request.getCustomersOrdersProductTable()) {
                            if (!saveCustomersOrdersProductTable(row, request.getId())) {//         //метод 2
                                break;
                            }
                            productIds = productIds + (productIds.length()>0?",":"") + row.getProduct_id();
                        }
                    }//удаление лишних строк
                    deleteCustomersOrdersProductTableExcessRows(productIds.length()>0?productIds:"0", request.getId());

                    //если завершается приемка - запись в историю товара
                    /*if(request.isIs_completed()){
                        Long myMasterId = userRepositoryJPA.getUserMasterIdByUsername(userRepository.getUserName());
                        for (CustomersOrdersProductForm row : request.getCustomersOrdersProductTable()) {
                            if (!addCustomersOrdersProductHistory(row, request, myMasterId)) {//         //метод 3
                                break;
    ...
    private Boolean updateCustomersOrdersWithoutTable(CustomersOrdersForm request) {
        ...
        try
        {
            ...
            stringQuery =   " update customers_orders set " +
                    ...
                    " fio = '" + (request.getFio() == null ? "" : request.getFio()) + "', " +
    ...
    private Boolean saveCustomersOrdersProductTable(CustomersOrdersProductForm row, Long customers_orders_id) {
            String stringQuery;
            String empty="";
            try {
                stringQuery =
                        ...
                        row.getNds_id() + ", " +
                        row.getReserved() + ", '" +
                        (row.getAdditional()!=null?row.getAdditional():empty) +
                        " ') " +
                        ...
                        " additional = " + (row.getAdditional()!=null?row.getAdditional():empty);
                Query query = entityManager.createNativeQuery(stringQuery);
                query.executeUpdate();
                return true;
            }
