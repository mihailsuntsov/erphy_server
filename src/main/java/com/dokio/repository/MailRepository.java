...
public class MailRepository {
    ...
    public Integer activateAccount(String email, String uuid){
        try{
            String mailLink ="http://localhost:4200/activate/"+ uuid ;
            ...
            MimeMessagePreparator preparator = getMessagePreparator("noreply@dokio.ru",email,"Dokio registration confirm",mailBody);
    ...
    public Integer forgotPass(String email){
        try{
            ...
            String mailLink ="http://localhost:4200/newpass/"+ uuid ;
            String mailBody="\n\n You have requested for a new password from the Dokio application. Please use the below link to set new password."+ "\n\n Click on Link: "+mailLink;
            MimeMessagePreparator preparator = getMessagePreparator("noreply@dokio.ru",email,"Password repairing",mailBody);
            mailSender.send(preparator);
            return 1;
        } catch (Exception e){
            e.printStackTrace();
